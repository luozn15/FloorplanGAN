...
class Generator(nn.Module):
    def __init__(self, dataset):# feature_size, class_num, element_num):
        ...
        self.encoder_fc0 = nn.Conv2d(4, dim*4,kernel_size=1,stride=1,bias=False) #4 是 (xc,w,yc,h) 的位数
        self.encoder_batch_norm0 = nn.BatchNorm2d(dim*4)
        self.encoder_fc1 = nn.Conv2d(4, dim,kernel_size=1,stride=1,bias=False) 
        self.encoder_batch_norm1 = nn.BatchNorm2d(dim)
        self.encoder_fc2 = nn.Conv2d(dim, dim,kernel_size=1,stride=1,bias=False) 
        self.encoder_batch_norm2 = nn.BatchNorm2d(dim)
        self.encoder_fc3 = nn.Conv2d(dim, dim*4,kernel_size=1,stride=1,bias=False) 
        self.encoder_batch_norm3 = nn.BatchNorm2d(dim*4)
        # Batchsize=16, Chanel=dim*4 ,S=8, O=10
        encoder_layer = nn.TransformerEncoderLayer(d_model=dim*4*self.class_num, nhead=8)
        ...
        self.decoder_fc0 = nn.Conv2d(dim*4, dim*4,kernel_size=1,stride=1,bias=False) 
        self.decoder_batch_norm0 = nn.BatchNorm2d(dim*4)
        self.decoder_fc1 = nn.Conv2d(dim*4, dim,kernel_size=1,stride=1,bias=False) 
        self.decoder_batch_norm1 = nn.BatchNorm2d(dim)
        self.decoder_fc2 = nn.Conv2d(dim, dim,kernel_size=1,stride=1,bias=False) 
        self.decoder_batch_norm2 = nn.BatchNorm2d(dim)
        self.decoder_fc3 = nn.Conv2d(dim, dim*4,kernel_size=1,stride=1,bias=False) 
        self.decoder_batch_norm3 = nn.BatchNorm2d(dim*4)

        self.decoder_fc4 = nn.Conv2d(dim*4,4,kernel_size=1,stride=1,bias=False)
    ...
    def forward(self, input_data, input_length):#(B,S,14),(B,S)
        ...
        # Encoder
        h0_0 = self.encoder_batch_norm0(self.encoder_fc0(modified))#(B,dim*4,S,10)
        h0_1 = F.relu(self.encoder_batch_norm1(self.encoder_fc1(modified)))#(B,dim,S,10)
        h0_2 = F.relu(self.encoder_batch_norm2(self.encoder_fc2(h0_1)))#(B,dim,S,10)
        h0_3 = self.encoder_batch_norm3(self.encoder_fc3(h0_2))#(B,dim*4,S,10)
        ...
        encoded = encoded.reshape(self.element_num,batch_size,-1)#(S,B,dim*4*10)
        ...
        transformed = self.transformer_encoder(src=encoded,src_key_padding_mask=input_length)#(S,B,dim*4*10)
        transformed = transformed.reshape(transformed.shape[0],transformed.shape[1],-1,self.class_num)#(S,B,dim*4,10)
        ...
        # Decoder
        h1_0 = self.decoder_batch_norm0(self.decoder_fc0(transformed))#(B,dim*4,S,10)
        h1_1 = F.relu(self.decoder_batch_norm1(self.decoder_fc1(transformed)))#(B,dim,S,10)
        h1_2 = F.relu(self.decoder_batch_norm2(self.decoder_fc2(h1_1)))#(B,dim,S,10)
        h1_3 = self.decoder_batch_norm3(self.decoder_fc3(h1_2))#(B,dim*4,S,10)
        decoded = F.relu(h1_0+h1_3)#(B,dim*4,S,10)

        decoded = F.relu(self.decoder_batch_norm4(self.decoder_fc4(decoded)))#(B,4,S,10)
...
class Generator2(nn.Module):
    ...
    def forward(self, input_data, input_length):#(B,S,14),(B,S)
        ...
        syn_geo = self.tanh_geo(self.branch_geo(decoded)) + geo #大跨residual connect#(B,S,4)
        ...
        # Remove redundancy
        res = syn.masked_fill(input_length.unsqueeze(-1).repeat(1,1,syn.shape[-1]),value=torch.tensor(0))
...
class WireframeDiscriminator(nn.Module):
    def __init__(self, dataset, rendered_size=64):
        ...
        self.cnn = nn.Sequential(
            nn.Conv2d(self.class_num,64,kernel_size=5,stride=2,padding=2,bias=False),#padding=2,same
            nn.BatchNorm2d(64),
        ...
        self.classifier = nn.Sequential(
            nn.Conv2d(256,1024,kernel_size=int(self.rendered_size/8),stride=1,bias=False),
    ...
    def render(self, elements):
        ...
        x = rects[:,:,0].reshape(batch_size,-1,1,1)*self.rendered_size #batch_size,46,1,1
        ...
        F_ = torch.sum(\
                    ...
                    .mul(rendered.reshape(batch_size,num_elements,self.rendered_size,self.rendered_size,1))\
...
def render(elements, rendered_size, class_num):
    ...
    x = rects[:,:,0].reshape(batch_size,-1,1,1)*rendered_size #batch_size,46,1,1
